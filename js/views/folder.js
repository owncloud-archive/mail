...
views.Folders = Backbone.Marionette.CollectionView.extend({
	...
	updateTitle: function() {
		...
		if ( unread > 0) {
			window.document.title = name + ' (' + unread + ') - ' + activeAccount.get('email') + ' - Mail - ' + oc_defaults.title;
		} else {
			window.document.title = name + ' - ' + activeAccount.get('email') + ' - Mail - ' + oc_defaults.title;
		}
	}
