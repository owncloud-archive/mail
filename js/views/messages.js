...
define(function(require) {
	var MessageList = require('models/messagelist'),
		...
		NoSearchResultMessageListView = require('views/nosearchresultmessagelist');
	...
	return Backbone.Marionette.CompositeView.extend({
		...
		loadNew: function() {
			if (!require('Mail').State.currentAccountId) {
				return;
			}
			if (!require('Mail').State.currentFolderId) {
				return;
			}
		...
		loadMessages: function(reload) {
			...
			var self = this;
			require('Mail').Communication.fetchMessageList(
				...
				require('Mail').State.currentFolderId,
				{
					...
					onSuccess: function(jsondata) {
						...
						require('Mail').UI.setMessageActive(require('Mail').State.currentMessageId);
					},
					onError: function() {
						require('Mail').UI.showError(t('mail', 'Error while loading messages.'));
						// Set the old folder as being active
						require('Mail').UI.setFolderActive(require('Mail').State.currentAccountId, require('Mail').State.currentFolderId);
					},
					onComplete: function() {
